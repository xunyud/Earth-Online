...
async function getWechatAccessToken(): Promise<string> {
  ...
  if (!appId || !appSecret) {
    throw new Error("Missing WECHAT_APP_ID or WECHAT_APP_SECRET env");
  }
  ...
  if (data.errcode) {
    throw new Error(
      `Failed to get access_token: errcode=${data.errcode} errmsg=${data.errmsg}`,
    );
  }
...
function resolvePushLanguage(body?: Record<string, unknown>) {
  const code = toText(body?.language_code).toLowerCase();
  if (code.startsWith("en")) return "en" as const;
  if (code.startsWith("zh")) return "zh" as const;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requestedLanguage = resolvePushLanguage(body);
    ...
    if (queryErr) {
      throw new Error(`Failed to query candidates: ${queryErr.message}`);
    }
    ...
    if (eligible.length === 0) {
      const reason = singleUserId
        ? localizedText(
          ...
          "This user is not eligible for push (WeChat not linked / push disabled / outside the 48-hour interaction window).",
        )
        : localizedText(
          requestedLanguage,
          "暂无符合条件的推送用户",
          "No eligible users are available for push.",
        );
      return new Response(
        JSON.stringify({ success: true, pushed: 0, message: reason }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    ...
    try {
      ...
      for (const u of eligible) {
        await supabase.from("push_logs").insert({
          ...
          status: "failed",
          error_message: `access_token failed: ${toErrorMessage(e)}`,
        });
      }
      throw e;
    }
    ...
    for (const user of eligible) {
      ...
      try {
        ...
        const summaryResp = await fetch(
          `${supabaseUrl}/functions/v1/weekly-summary`,
          {
            ...
            body: JSON.stringify({
              ...
              is_english: requestedLanguage === "en",
            }),
          },
        );
        ...
        if (!summaryData.success || !summaryData.summary) {
          throw new Error(
            summaryData.error || "Weekly summary generation failed",
          );
        }
        const summaryLanguage: PushLanguage = summaryData.language === "en"
          ? "en"
          : requestedLanguage;

        // 3b. Markdown → 纯文本，截断到 600 字
        const plainText = stripMarkdown(summaryData.summary);
        const truncated = plainText.length > 600
          ? plainText.slice(0, 597) + "..."
          : plainText;
        const messageTitle = localizedText(
          summaryLanguage,
          "本周冒险周报",
          "Weekly Adventure Report",
        );
        const messageContent = `📊 ${messageTitle}\n\n${truncated}`;

        // 3c. 发送微信客服消息
        const sendResult = await sendWechatTextMessage(
          accessToken,
          openId,
          messageContent,
        );
