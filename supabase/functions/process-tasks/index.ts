...
const openai = new OpenAI({
  ...
  baseURL: normalizeOpenAIBaseUrl(
    Deno.env.get("OPENAI_BASE_URL") || "https://api.86gamestore.com",
  ),
});
      ...
      messages: [
        {
          role: "system",
          content:
            `You are the "Quest Master" AI of an RPG game. Your job is to analyze chaotic chat logs from the user and synthesize them into a structured Quest Tree.
              ...
              quests: {
                ...
                items: {
                  ...
                  properties: {
                    temp_id: {
                      type: "string",
                      description: "Temporary unique ID for linking",
                    },
                    parent_temp_id: {
                      type: "string",
                      nullable: true,
                      description: "Temporary ID of parent node",
                    },
                    title: { type: "string" },
                    quest_tier: {
                      type: "string",
                      enum: ["Main_Quest", "Side_Quest", "Daily"],
                    },
        ...
        if (messages.length > 0) {
          ...
          if (extractedQuests.length > 0) {
            ...
            const questsToInsert = extractedQuests.map((q) => ({
              ...
              user_id: userId,
              parent_id: q.parent_temp_id
                ? tempIdMap.get(q.parent_temp_id)
                : null,
              title: q.title,
            ...
            // Insert roots
            if (roots.length > 0) {
              const { error: rootError } = await supabase.from("quest_nodes")
                .insert(roots);
            ...
            if (children.length > 0) {
              const { error: childError } = await supabase.from("quest_nodes")
                .insert(children);
