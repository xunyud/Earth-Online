...
export function buildEventFallback(
  ...
  if (hasAny(joined, /夜间|熬夜|高强度|疲劳|睡眠不足/)) {
    return {
      title: "恢复支线：热水澡 + 10 分钟音乐",
      description: "你最近推进很猛。今晚先降噪恢复，完成后再决定是否继续主线。",
      reward_xp: 30,
      reward_gold: 60,
      reason: "依据近期高强度信号生成。",
    };
  ...
  if (hasAny(joined, /连续|稳定|清盘|连胜/)) {
    return {
      title: "稳态支线：明日三件事预排",
      description: "把明天最重要的三件事写出来并排序，降低启动成本。",
      reward_xp: 25,
      reward_gold: 40,
      reason: "依据连续推进信号生成。",
    };
  ...
  return {
    title: "轻量支线：步行 15 分钟并补水",
    description: "先恢复体能，再回到任务，长期效率会更稳定。",
    reward_xp: 20,
    reward_gold: 35,
    reason: "依据近期任务节奏生成。",
  };
...
export function buildChatFallback(
  ...
  return {
    ...
    suggested_task: wantsRecovery
      ? {
        title: "恢复支线：拉伸 8 分钟 + 补水",
        description: "先恢复身体状态，再继续推进主线任务。",
        xp_reward: 22,
        quest_tier: "Daily",
      }
      : undefined,
...
export function buildNightReflectionFallback(
  ...
  return {
    ...
    suggested_task: {
      title: "明日恢复支线：晨间拉伸 10 分钟",
      description: "降低疲劳堆积，保证明天主线推进的启动质量。",
      xp_reward: 24,
      quest_tier: "Daily",
    },
  };
...
export async function generateDailyEvent(memory: GuideMemoryBundle) {
  ...
  const systemPrompt =
    "你是游戏GM。请根据用户近期与长期记忆生成一个每日突发事件任务。任务必须具体可执行，避免空话。只输出JSON。";
  const userPrompt = `
请生成每日突发事件，字段如下：
{
  "title":"...",
...
export async function generateChat(
  ...
  const systemPrompt =
    "你是地球Online专属向导。请基于用户记忆回复，不要模板化寒暄。保持短句，给出可执行建议。只输出JSON。";
  const userPrompt = `
场景: ${scene}
用户消息: ${message}
记忆:
...
export async function generateNightReflection(
  ...
  const systemPrompt =
    "你是地球Online夜间向导。请做两轮复盘：先总结今天，再问一个关怀问题，并给出明日建议任务。只输出JSON。";
  const userPrompt = `
目标日期: ${dayId}
记忆:
${memory.packed_context}
