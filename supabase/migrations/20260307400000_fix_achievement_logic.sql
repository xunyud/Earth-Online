-- 修复成就检查逻辑：
-- 1. total_completed 不排除已删除任务（累计完成不应因删除而倒退）
-- 2. board_clear 增加真实条件校验（v_total_completed >= 1）
-- 3. first_wechat 增加真实条件校验（last_wechat_interaction IS NOT NULL）
...
DECLARE
    ...
    rec RECORD;
BEGIN
    -- quest / special 类别：统计累计完成任务数（含已删除的，防止倒退）
    ...
    LOOP
        IF (rec.condition_type = 'total_completed' AND v_total_completed >= rec.condition_value)
           ...
           OR (rec.condition_type = 'board_clear'     AND v_total_completed >= rec.condition_value)
